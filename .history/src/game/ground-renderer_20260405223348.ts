...
export class GroundRenderer {
  ...
  private async loadGLTF(path: string): Promise<{ group: THREE.Group; animations: THREE.AnimationClip[] }> {
    ...
    // Normalize to 2-unit height
    const box = new THREE.Box3().setFromObject(scene);
    const size = box.getSize(new THREE.Vector3());
    const maxDim = Math.max(size.x, size.y, size.z);
    if (maxDim > 0) scene.scale.setScalar(2.0 / maxDim);
  ...
  private async loadPlayerModel(): Promise<void> {
    ...
    try {
      ...
      this.applyCharacterMaterial(model, tintHex);
  ...
  private async loadEnemyModel(enemy: GroundEnemy): Promise<THREE.Group> {
    ...
    try {
      ...
      // Apply enemy tint as solid colored material (same as player pipeline)
      this.applyCharacterMaterial(model, cfg.tint, 0.18);
      return model;
    } catch {
      return this.createPlaceholder(cfg.tint);
    }
