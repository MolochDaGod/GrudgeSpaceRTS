...
export function SmallPanel({ title, children, style }: {
  title?: string; children: ReactNode; style?: CSSProperties;
}) {
  return (
    <div style={{
      backgroundImage: `url(${H}/Shop_Box.png)`, backgroundSize: '100% 100%',
      padding: '10px 14px 14px', position: 'relative',
      ...style,
    }}>
      {title && (
        <div style={{
          fontSize: 11, fontWeight: 800, color: '#b8e0d0', letterSpacing: 2,
...
export function Btn({ label, onClick, disabled, active, wide, icon, style, children }: {
  label?: string; onClick?: () => void; disabled?: boolean; active?: boolean;
  wide?: boolean; icon?: string; style?: CSSProperties; children?: ReactNode;
}) {
  const [hovered, setHovered] = useState(false);
  const prefix = wide ? 'Long' : '';
  const src = disabled ? `${H}/Disable_${prefix}Btn.png`
    : (hovered || active) ? `${H}/Hover_${prefix}Btn.png`
  ...
  return (
    <div
      ...
      style={{
        ...
        minWidth: wide ? 140 : 64, height: wide ? 36 : 40,
    ...
    >
      ...
      {icon && <img src={icon} alt="" style={{
        width: 18, height: 18, position: 'relative', zIndex: 1, marginRight: label ? 4 : 0,
        imageRendering: 'auto',
      }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />}
      {label && <span style={{
        position: 'relative', zIndex: 1, fontSize: wide ? 11 : 9, fontWeight: 700,
        color: active ? '#fff' : disabled ? '#556' : '#c8e8dc',
        letterSpacing: 1, textShadow: '0 1px 3px rgba(0,0,0,0.7)',
      }}>{label}</span>}
...
export function Slot({ children, size, onClick, active, style }: {
  children?: ReactNode; size?: number; onClick?: (e: React.MouseEvent) => void;
  active?: boolean; style?: CSSProperties;
}) {
  const sz = size ?? 52;
  return (
    <div onClick={onClick} style={{
      position: 'relative', width: sz, height: sz, cursor: onClick ? 'pointer' : 'default',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...style,
    }}>
      <img src={`${H}/InventorySlot.png`} alt="" style={{
        position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'fill',
...
export function Bar({ value, max, color, height, width, label }: {
  ...
  return (
    <div style={{ position: 'relative', width: width ?? '100%', height: h }}>
      <img src={`${H}/HealthBar_Line.png`} alt="" style={{
        position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'fill',
        opacity: 0.6,
      }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
      <div style={{
...
export function Frame({ title, children, onClose, width, style }: {
  title?: string; children: ReactNode; onClose?: () => void;
  width?: number | string; style?: CSSProperties;
}) {
  return (
    <div style={{
      position: 'relative', width: width ?? 'auto',
      backgroundImage: `url(${H}/Box_Item.png)`, backgroundSize: '100% 100%',
      padding: '36px 16px 16px', minHeight: 80,
      boxShadow: '0 8px 40px rgba(0,0,0,0.5)',
      ...style,
    }}>
      {title && (
        <div style={{
          position: 'absolute', top: 6, left: 40, right: 40,
          textAlign: 'center', fontSize: 12, fontWeight: 800,
          color: '#b8f0e0', letterSpacing: 2, textTransform: 'uppercase',
...
export function ResBox({ icon, value, rate, color }: {
  icon: ReactNode; value: number; rate?: number; color: string;
}) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 4,
      backgroundImage: `url(${H}/BgSettingSmallBox.png)`, backgroundSize: '100% 100%',
      padding: '3px 10px 3px 6px', borderRadius: 4, minWidth: 90,
    }}>
      ...
      {rate != null && rate > 0 && (
        <span style={{ fontSize: 8, color: '#44dd88', opacity: 0.8, marginLeft: 2 }}>
          +{Math.round(rate)}
        </span>
      )}
    </div>
...
export function Img({ src, alt, style, fallback }: {
