...
import {
  TEAM_COLORS, PLANET_TYPE_DATA,
  SHIP_ROLE_LABELS, SHIP_ROLE_COLORS, SHIP_ROLES,
  COMMANDER_SPEC_LABEL,
  type TacticalOrder, type Fleet,
} from './space-types';
...
const RoleBadge = ({ role }: { role: string }) => {
  ...
  return (
    <span style={{ fontSize: 8, padding: '1px 5px', borderRadius: 3, background: `${color}22`,
      border: `1px solid ${color}55`, color, letterSpacing: 0.5, fontWeight: 700, whiteSpace: 'nowrap' }}>
      {label.toUpperCase()}
    </span>
  );
};
...
export function StarMapOverlay({ renderer, onClose }: { renderer: SpaceRenderer; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: C.bg, zIndex: 200,
      display: 'flex', flexDirection: 'column', fontFamily: "'Segoe UI', monospace" }}>

      {/* ── Header ─────────────────────────────────────────────── */}
      <div style={{ display: 'flex', alignItems: 'center', padding: '10px 20px',
        borderBottom: `1px solid ${C.border}`, background: 'rgba(2,6,18,0.98)', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginLeft: 24 }}>
          {(['overview','fleets','tactics','commanders'] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              padding: '6px 18px', border: `1px solid ${tab===t ? C.accent : C.border}`,
              borderRadius: 5, cursor: 'pointer', fontSize: 11, fontWeight: 700, letterSpacing: 1,
              background: tab===t ? 'rgba(68,136,255,0.18)' : 'transparent',
              color: tab===t ? C.accent : C.muted, transition: 'all 0.15s',
            }}>
              <Icon src={`/assets/space/ui/hud/Ability0${(['overview','fleets','tactics','commanders'] as Tab[]).indexOf(t)+7}.png`} size={14} />
              {' '}{t.toUpperCase()}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 16, fontSize: 10, color: C.muted }}>
          <span>⏱ {Math.floor(state.gameTime/60)}:{String(Math.floor(state.gameTime%60)).padStart(2,'0')}</span>
          <span style={{ color: C.accent }}>🌍 {state.planets.filter(p=>p.owner===1).length}/{state.planets.length} planets</span>
        ...
        <button onClick={onClose} style={{
          background: 'transparent', border: `1px solid ${C.border}`, color: '#888',
          fontSize: 16, padding: '4px 12px', borderRadius: 5, cursor: 'pointer', marginLeft: 8,
        }}>✕ [M]</button>
      </div>
...
function OverviewTab({ renderer }: { renderer: SpaceRenderer }) {
  ...
  const CANVAS_W = 680, CANVAS_H = 560;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      bg.addColorStop(0, '#02060e'); bg.addColorStop(1, '#010208');
      ctx.fillStyle = bg; ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);

      // Grid lines
      ctx.strokeStyle = 'rgba(68,136,255,0.06)'; ctx.lineWidth = 1;
      for (let i = 0; i < CANVAS_W; i += 68) { ctx.beginPath(); ctx.moveTo(i,0); ctx.lineTo(i,CANVAS_H); ctx.stroke(); }
      for (let j = 0; j < CANVAS_H; j += 56) { ctx.beginPath(); ctx.moveTo(0,j); ctx.lineTo(CANVAS_W,j); ctx.stroke(); }

      // Resource nodes
      for (const [,nd] of state.resourceNodes) {
        const { x, y } = toCanvas(nd.x, nd.y);
        const nc = nd.kind === 'moon' ? '#998877' : nd.kind === 'asteroid' ? '#554433'
          : nd.kind === 'ice_rock' ? '#88ccee' : '#44ffcc';
      ...
      for (const [, fleet] of state.fleets) {
        ...
        ctx.setLineDash([4, 4]);
        ctx.strokeStyle = fleet.color; ctx.lineWidth = 1; ctx.globalAlpha = 0.5;
      ...
      for (const p of state.planets) {
        ...
        // Owner ring
        if (p.owner !== 0) {
          ctx.strokeStyle = hx(ownerColor); ctx.lineWidth = 2.5; ctx.globalAlpha = 0.8;
        ...
        if (p.captureProgress > 0 && p.captureTeam !== 0) {
          ...
          ctx.strokeStyle = hx(captureColor); ctx.lineWidth = 2; ctx.globalAlpha = 0.9;
          ctx.beginPath();
      ...
      for (const [,s] of state.ships) {
        ...
        if (s.selected) { ctx.strokeStyle = '#fff'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(x,y,sz+1.5,0,Math.PI*2); ctx.stroke(); }
      }

      // Camera viewport rect
      const cam = renderer.controls.cameraState;
      const vw = CANVAS_W * 0.12, vh = CANVAS_H * 0.12;
  ...
  const selShips = sel ? [...state.ships.values()].filter(s=>!s.dead&&s.team===1&&
    Math.sqrt((s.x-sel.x)**2+(s.y-sel.y)**2) < sel.captureRadius * 2) : [];
      ...
      <div style={{ position:'relative', flexShrink:0 }}>
        ...
        {flash && (
          <div style={{ position:'absolute', top:8, left:'50%', transform:'translateX(-50%)',
            background:'rgba(68,136,255,0.8)', padding:'4px 14px', borderRadius:4,
            fontSize:10, color:'#fff', fontWeight:700 }}>{flash}</div>
        )}
        {/* Legend */}
        <div style={{ position:'absolute', bottom:8, left:8, display:'flex', gap:10,
          background:'rgba(2,6,18,0.85)', padding:'4px 8px', borderRadius:4, fontSize:8 }}>
          {Object.entries(SHIP_ROLE_COLORS).map(([r,c]) => (
            <span key={r} style={{ color:c }}>{SHIP_ROLE_LABELS[r as keyof typeof SHIP_ROLE_LABELS]}</span>
          ))}
        ...
        {sel ? (
          <>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10,
              ...
              <div style={{ width:16, height:16, borderRadius:'50%', background:hx(sel.color),
                boxShadow:`0 0 8px ${hx(sel.color)}88`, flexShrink:0 }} />
              <div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:5, marginBottom:10 }}>
              {[
                { label:'Credits/s', v: Math.round(sel.resourceYield.credits * PLANET_TYPE_DATA[sel.planetType].resourceMult.credits), c:'#fc4' },
                { label:'Energy/s',  v: Math.round(sel.resourceYield.energy  * PLANET_TYPE_DATA[sel.planetType].resourceMult.energy),  c:'#4df' },
                { label:'Minerals/s',v: Math.round(sel.resourceYield.minerals* PLANET_TYPE_DATA[sel.planetType].resourceMult.minerals),c:'#4f8' },
              ].map(({label,v,c}) => (
                <div key={label} style={{ padding:'5px 8px', border:`1px solid ${C.border}`, borderRadius:5,
            ...
            {techTree && (
              <div style={{ marginBottom:10 }}>
                <div style={{ fontSize:10, color:C.muted, letterSpacing:1, marginBottom:5 }}>
                  TECH: {techTree.name}
                </div>
                <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                  {techTree.nodes.map(n => {
                    ...
                    return (
                      <div key={n.id} style={{ padding:'2px 6px', borderRadius:3, fontSize:8,
            ...
            {selShips.length > 0 && (
              <div>
                <div style={{ fontSize:10, color:C.muted, letterSpacing:1, marginBottom:5 }}>
                  NEARBY SHIPS ({selShips.length})
                </div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
                  {selShips.slice(0,20).map(s => {
                    const role = SHIP_ROLES[s.shipType];
                    return (
                      <div key={s.id} style={{ padding:'3px 6px', border:`1px solid ${C.border}`,
                        borderRadius:4, fontSize:8, background:C.panel,
            ...
            }} style={{ marginTop:10, padding:'5px 14px', background:C.accent, border:'none',
              borderRadius:4, color:'#fff', fontSize:10, cursor:'pointer', fontWeight:700 }}>
              📍 JUMP CAMERA HERE
            </button>
          </>
        ) : (
              ...
              return (
                <div key={p.id} onClick={() => setSelectedPlanet(p)}
                  style={{ padding:'8px 10px', marginBottom:4, borderRadius:6, cursor:'pointer',
                    border:`1px solid ${C.border}`, background:C.panel, display:'flex', alignItems:'center', gap:8,
                    transition:'all 0.1s' }}>
                  <div style={{ width:12, height:12, borderRadius:'50%', background:hx(p.color),
                    boxShadow:`0 0 6px ${hx(p.color)}88`, flexShrink:0 }} />
                  <div style={{ flex:1 }}>
  ...
  return (
    <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ width:260, borderRight:`1px solid ${C.border}`, padding:'12px', overflowY:'auto' }}>
        ...
        {/* Create */}
        <div style={{ marginBottom:14, padding:'8px', border:`1px solid ${C.border}`, borderRadius:6,
          background:C.panel }}>
          ...
          <input value={newName} onChange={e=>setNewName(e.target.value)}
            ...
            style={{ width:'100%', padding:'5px 8px', background:'rgba(10,20,40,0.8)',
              border:`1px solid ${C.border}`, borderRadius:4, color:C.text, fontSize:11, marginBottom:6 }}
          />
          ...
          <button onClick={createFleet}
            style={{ width:'100%', padding:'5px', background:C.accent, border:'none', borderRadius:4,
        ...
        {fleets.length === 0
          ...
          : fleets.map(fl => {
            const alive = fl.shipIds.filter(id => { const s=state.ships.get(id); return s&&!s.dead; }).length;
            return (
              <div key={fl.name} onClick={() => setSelectedFleet(fl.name===selectedFleet?null:fl.name)}
                style={{ padding:'8px 10px', marginBottom:4, borderRadius:6, cursor:'pointer',
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <span style={{ fontSize:9, color:fl.color }}>{alive}/{fl.shipIds.length}</span>
                </div>
        ...
        {sel ? (
          <>
            ...
            <div style={{ display:'flex', gap:6, marginBottom:12 }}>
              <button onClick={() => assignSelected(sel.name)}
                style={{ padding:'5px 12px', background:'rgba(68,136,255,0.2)',
                  border:`1px solid ${C.accent}`, borderRadius:4, color:C.accent,
                  fontSize:10, cursor:'pointer', fontWeight:700 }}>
                + ADD SELECTED SHIPS
              </button>
              {state.planets.filter(p=>p.owner===1).map(p => (
                <button key={p.id}
                  onClick={() => {
                    const fl = state.fleets.get(sel.name);
                    if (fl) { fl.rallyPlanetId = p.id; fl.order = 'attack'; fl.orderTargetPlanetId = p.id; }
                  }}
                  style={{ padding:'5px 10px', background:C.panel, border:`1px solid ${C.border}`,
                    borderRadius:4, color:C.muted, fontSize:9, cursor:'pointer' }}>
                  Rally → {p.name}
                </button>
              ))}
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:6 }}>
              {(selShips as (import('./space-types').SpaceShip | undefined)[]).map(s => {
                ...
                return (
                  <div key={s.id} style={{ padding:'7px 9px', border:`1px solid ${C.border}`,
                    borderRadius:6, background:C.panel, opacity: s.dead?0.35:1 }}>
...
function TacticsTab({ renderer }: { renderer: SpaceRenderer }) {
  ...
  const [form, setForm] = useState<Partial<TacticalOrder>>({
    ...
    trigger:'planet_attacked', triggerPlanetId:null, triggerFleetName:null,
    action:'attack_planet', actionPlanetId:null, fleetName:null,
    priority:3, cooldown:60,
  });
    ...
    <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ width:320, borderRight:`1px solid ${C.border}`, padding:'12px', overflowY:'auto' }}>
        ...
        <div style={{ marginBottom:8 }}>
          ...
          <input value={form.label??''} onChange={e=>S('label',e.target.value)}
            style={{ width:'100%', padding:'5px 8px', marginTop:2,
              background:'rgba(10,20,40,0.8)', border:`1px solid ${C.border}`,
        ...
        <div style={{ marginBottom:8 }}>
          ...
          <select value={form.trigger} onChange={e=>S('trigger',e.target.value)}
            style={{ width:'100%', padding:'5px 8px', marginTop:2,
              background:'rgba(10,20,40,0.95)', border:`1px solid ${C.border}`,
        ...
        {(form.trigger==='planet_attacked'||form.trigger==='planet_captured') && (
          <div style={{ marginBottom:8 }}>
            ...
            <select value={form.triggerPlanetId??''} onChange={e=>S('triggerPlanetId',+e.target.value||null)}
              style={{ width:'100%', padding:'5px 8px', marginTop:2,
                background:'rgba(10,20,40,0.95)', border:`1px solid ${C.border}`,
                borderRadius:4, color:C.text, fontSize:10 }}>
        ...
        {form.trigger==='fleet_below_half' && (
          <div style={{ marginBottom:8 }}>
            ...
            <select value={form.triggerFleetName??''} onChange={e=>S('triggerFleetName',e.target.value||null)}
              style={{ width:'100%', padding:'5px 8px', marginTop:2,
                background:'rgba(10,20,40,0.95)', border:`1px solid ${C.border}`,
                borderRadius:4, color:C.text, fontSize:10 }}>
              <option value="">— Select fleet —</option>
              {fleetNames.map(n=><option key={n} value={n}>{n}</option>)}
            </select>
          </div>
        )}
        ...
        {(form.action==='attack_planet'||form.action==='defend_planet'||form.action==='rally_fleet') && (
          <div style={{ marginBottom:8 }}>
            ...
            <select value={form.actionPlanetId??''} onChange={e=>S('actionPlanetId',+e.target.value||null)}
              style={{ width:'100%', padding:'5px 8px', marginTop:2,
                background:'rgba(10,20,40,0.95)', border:`1px solid ${C.border}`,
                borderRadius:4, color:C.text, fontSize:10 }}>
              <option value="">— Select planet —</option>
              {state.planets.map(p=><option key={p.id} value={p.id}>{p.name} ({p.owner===1?'ours':p.owner?'enemy':'neutral'})</option>)}
            </select>
          </div>
        )}
        ...
        <div style={{ marginBottom:8 }}>
          ...
          <select value={form.fleetName??''} onChange={e=>S('fleetName',e.target.value||null)}
            style={{ width:'100%', padding:'5px 8px', marginTop:2,
              background:'rgba(10,20,40,0.95)', border:`1px solid ${C.border}`,
              borderRadius:4, color:C.text, fontSize:10 }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:12 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <input type="number" min={10} value={form.cooldown??60}
        ...
        <button onClick={submit} style={{ width:'100%', padding:'8px', background:C.accent,
          border:'none', borderRadius:5, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}>
          ➕ ADD ORDER
        </button>
      </div>
      ...
      <div style={{ flex:1, padding:'12px', overflowY:'auto' }}>
        ...
        {state.tacticalOrders.length === 0 ? (
          ...
          <div key={order.id} style={{ padding:'10px 12px', marginBottom:6, borderRadius:8,
            border:`1px solid ${order.enabled?C.accent:C.border}`,
            background: order.enabled?'rgba(68,136,255,0.06)':C.panel }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontSize:12, fontWeight:700, color:order.enabled?'#fff':'#555', flex:1 }}>
                {order.label}
              </span>
              <span style={{ fontSize:9, padding:'1px 5px', borderRadius:2, background:`rgba(68,136,255,0.2)`,
                color:C.accent }}>P{order.priority}</span>
              <button onClick={() => {
                  const o = state.tacticalOrders.find(x=>x.id===order.id);
                  if (o) o.enabled = !o.enabled;
                }}
                style={{ fontSize:9, padding:'2px 8px', border:`1px solid ${C.border}`, borderRadius:3,
                  background:'transparent', color:order.enabled?C.green:'#666', cursor:'pointer' }}>
                {order.enabled?'ON':'OFF'}
              </button>
              <button onClick={() => renderer.engine.removeTacticalOrder(order.id)}
                style={{ fontSize:9, padding:'2px 8px', border:'1px solid #442222', borderRadius:3,
                  background:'transparent', color:'#f44', cursor:'pointer' }}>
                ✕
              </button>
            </div>
...
function CommandersTab({ renderer }: { renderer: SpaceRenderer }) {
  ...
  return (
    <div style={{ padding:'14px', overflowY:'auto', height:'100%' }}>
      <div style={{ fontSize:13, fontWeight:700, color:C.accent, marginBottom:14, letterSpacing:2 }}>
        COMMANDER CORPS · ALL TEAMS
      </div>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        {my.length === 0
          ? <div style={{ fontSize:10, color:C.muted }}>No commanders. Recruit from the CMDR panel in-game.</div>
          : (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:8 }}>
              {my.map(cmd => {
                const equippedShip = cmd.equippedShipId ? state.ships.get(cmd.equippedShipId) : null;
                return (
                  <div key={cmd.id} style={{ padding:'10px', border:`1px solid ${
                    cmd.state==='onship'?'#2244aa':cmd.state==='training'?'#444400':'#1a3a22'}`,
                    borderRadius:8, background:C.panel }}>
                    <div style={{ display:'flex', gap:8, marginBottom:6 }}>
                      <img src={cmd.portrait} alt={cmd.name}
                        style={{ width:42, height:42, borderRadius:5, objectFit:'cover',
                          border:'1px solid #1a3a22' }}
                        onError={e=>{(e.target as HTMLImageElement).style.display='none'}}
                      />
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:12, fontWeight:700, color:'#fff' }}>{cmd.name}</div>
                        <div style={{ fontSize:9, color:C.green }}>{COMMANDER_SPEC_LABEL[cmd.spec]} · Lv {cmd.level}</div>
                        <div style={{ display:'flex', gap:2, marginTop:2 }}>
                          {Array.from({length:5},(_,i)=>(
                            <span key={i} style={{ fontSize:9, color:i<cmd.level?'#fc0':'#1a3a1a' }}>★</span>
                          ))}
                        </div>
                      </div>
                    </div>
                    <div style={{ fontSize:8, color: cmd.state==='training'?'#fc4':cmd.state==='onship'?'#4df':'#4f4',
                      marginBottom:4 }}>
                      {cmd.state==='training'?`⚙ Training (${Math.ceil(cmd.trainingTimeRemaining)}s)`:
                       cmd.state==='onship'?`🔵 Aboard: ${equippedShip?.shipType??'Unknown'}`:
      ...
      {/* Void Powers status */}
      <div>
        <div style={{ fontSize:10, color:'#aa44ff', marginBottom:8, letterSpacing:1, fontWeight:700 }}>
          VOID POWER STATUS
        </div>
        <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
          {Object.values(VOID_POWERS).map(pwr => {
            ...
            return (
              <div key={pwr.id} style={{ padding:'6px 10px', border:`1px solid ${unlocked?'#443366':'#1a2030'}`,
                borderRadius:6, background:unlocked?'rgba(80,30,100,0.3)':C.panel, width:130 }}>
                <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:3 }}>
                  <img src={pwr.icon} alt="" style={{ width:18, height:18, imageRendering:'pixelated' }}
                    onError={e=>{(e.target as HTMLImageElement).style.display='none'}} />
                  <span style={{ fontSize:10, fontWeight:700, color:unlocked?'#cc88ff':'#444' }}>
                    {pwr.name}
                  </span>
                </div>
