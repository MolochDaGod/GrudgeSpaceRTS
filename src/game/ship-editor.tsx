/**
 * ship-editor.tsx ΓÇö Ship Forge: Voxel spaceship builder
 *
 * 3D voxel editor using Three.js. Players design a custom hero ship
 * on a 32├ù16├ù32 grid, then export as GLB and save to their account.
 * One hero ship per account ΓÇö used in-game at the starting home world.
 */
...
// ΓöÇΓöÇ Constants ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const GRID_W = 32;
const GRID_H = 16;
const GRID_D = 32;
const CELL = 1.0;
...
interface UndoEntry {
  changes: Array<{ key: string; prev: VoxelType | null }>;
}
const MAX_UNDO = 80;
...
// ΓöÇΓöÇ Editor Component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
export function ShipForgeEditor({ onBack }: { onBack: () => void }) {
  ...
  const placedPartsRef = useRef<PlacedPart[]>([]); // mirror for Three.js callbacks
  ...
  // ΓöÇΓöÇ Check for existing ship on mount ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Three.js setup ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    ...
    // Right-click to orbit, middle-click to pan ΓÇö left-click reserved for voxel tools
    ...
    const groundPlane = new THREE.Mesh(new THREE.PlaneGeometry(GRID_W + 2, GRID_D + 2), new THREE.MeshBasicMaterial({ visible: false }));
    ...
    // ΓöÇΓöÇ Transform Gizmo ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const gizmo = new TransformControls(camera, renderer.domElement);
    gizmo.setSize(0.8);
    gizmo.addEventListener('dragging-changed', (event: any) => {
      controls.enabled = !event.value; // disable orbit while dragging gizmo
    });
  ...
  // ΓöÇΓöÇ Voxel mesh management ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    switch (type) {
      case 1:
        return new THREE.MeshStandardMaterial({
          color: new THREE.Color(r * 0.9, g * 0.9, b * 0.9),
      ...
      case 2:
        return new THREE.MeshStandardMaterial({ color: new THREE.Color(r * 0.45, g * 0.45, b * 0.45), roughness: 0.8, metalness: 0.4 });
      case 3:
        return new THREE.MeshStandardMaterial({
          color: new THREE.Color(0.05, 0.4, 0.9),
          emissive: new THREE.Color(0.0, 0.5, 1.0),
      ...
      case 5:
        return new THREE.MeshStandardMaterial({
          color: new THREE.Color(r * 0.6, g * 0.6, b * 0.6),
  ...
  function addVoxelMesh(ctx: NonNullable<typeof sceneRef.current>, x: number, y: number, z: number, type: VoxelType) {
    const k = key(x, y, z);
    // Remove existing if overwriting
    const existing = ctx.voxels.get(k);
    if (existing) {
      ctx.scene.remove(existing.mesh);
  ...
  // ΓöÇΓöÇ Raycast to find grid position ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Undo support ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  const handleUndo = useCallback(() => {
    ...
    for (const { key: k, prev } of entry.changes) {
      const [x, y, z] = k.split(',').map(Number);
      if (prev === null) {
        // Was created ΓÇö remove it
        removeVoxelMesh(ctx, x, y, z);
        voxelDataRef.current.delete(k);
      } else {
        // Was changed/erased ΓÇö restore previous type
  ...
  // ΓöÇΓöÇ Attach/detach gizmo when selection changes ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    const ctx = sceneRef.current;
    if (!ctx) return;
    if (selectedPlacedId !== null) {
      const part = placedPartsRef.current.find((p) => p.id === selectedPlacedId);
      if (part) {
        ctx.gizmo.attach(part.group);
        ctx.gizmo.setMode(gizmoMode);
      }
    } else {
      ctx.gizmo.detach();
    }
  }, [selectedPlacedId, gizmoMode]);

  // ΓöÇΓöÇ Keyboard shortcuts ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if ((e.target as HTMLElement)?.tagName === 'INPUT') return;
      const k = e.key.toLowerCase();
      // Voxel mode shortcuts
      ...
      // Gizmo mode shortcuts (prefab mode)
      else if (k === 'g') setGizmoMode('translate');
      else if (k === 'r') {
        if (mode === 'prefab') setGizmoMode('rotate');
        else setTool('paint');
      } else if (k === 's' && mode === 'prefab') {
        e.preventDefault();
        setGizmoMode('scale');
      } else if (k === 'e') {
        if (mode === 'voxel') setTool('erase');
      } else if (k === 'escape') {
        setSelectedPlacedId(null);
        setSelectedPartId(null);
      } else if (k === 'delete' || k === 'backspace') {
        // Delete selected placed part
        if (selectedPlacedId !== null) {
          const ctx = sceneRef.current;
          const part = placedPartsRef.current.find((p) => p.id === selectedPlacedId);
          if (ctx && part) {
            ctx.gizmo.detach();
            ctx.scene.remove(part.group);
          }
          setPlacedParts((prev) => {
            const next = prev.filter((p) => p.id !== selectedPlacedId);
            placedPartsRef.current = next;
            return next;
          });
          setSelectedPlacedId(null);
        }
  ...
  }, [handleUndo, mode, selectedPlacedId]);

  // ΓöÇΓöÇ Mouse handlers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Prefab placement handler ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const handlePrefabClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (partHits.length > 0 && !selectedPartId) {
        ...
        if (match) {
          setSelectedPlacedId(match.partId);
          return;
        }
      }
      ...
      if (partDef.glbPath) {
        // 3D model ΓÇö load async with correct loader, place immediately with placeholder
        ...
        if (isFbx) {
          ...
          gltfLoader.current.load(partDef.glbPath, (gltf) => onModelLoaded(gltf.scene), undefined, onError);
        }
      ...
      setPlacedParts((prev) => {
        const next = [...prev, { id, partId: selectedPartId, group, position: pos, rotationY: 0, scale: 1 }];
        placedPartsRef.current = next;
        return next;
      });
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // If mouse moved significantly since mousedown, it was an orbit drag ΓÇö ignore
      if (mouseDownPos.current) {
        const dx = e.clientX - mouseDownPos.current.x;
        const dy = e.clientY - mouseDownPos.current.y;
        if (dx * dx + dy * dy > 16) {
  ...
  // ΓöÇΓöÇ Export to GLB (includes both voxels AND placed prefab parts) ΓöÇΓöÇ
  const exportGLB = useCallback(async (): Promise<Blob | null> => {
    const ctx = sceneRef.current;
    const hasVoxels = voxelDataRef.current.size > 0;
    const hasParts = placedPartsRef.current.length > 0;
    if (!ctx || (!hasVoxels && !hasParts)) return null;

    const exportScene = new THREE.Scene();
    const group = new THREE.Group();

    // Centre around bounding box of ALL content
    const allBB = new THREE.Box3();
    ...
    for (const [k, type] of voxelDataRef.current) {
      ...
      mesh.position.set(x + 0.5, y + 0.5, z + 0.5);
      group.add(mesh);
      allBB.expandByPoint(mesh.position);
    }

    // Add placed prefab parts (clone their groups)
    for (const part of placedPartsRef.current) {
    ...
    // Centre everything
    const center = allBB.getCenter(new THREE.Vector3());
  ...
  // ΓöÇΓöÇ Save handler ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const handleSave = useCallback(async () => {
    if (voxelDataRef.current.size === 0 && placedPartsRef.current.length === 0) {
      setMessage('Place some parts or voxels first!');
      return;
    }
    if (!shipName.trim()) {
    ...
    if (ok) {
      ...
      setMessage('Save failed ΓÇö try again.');
    }
  }, [shipName, exportGLB, onBack]);

  // ΓöÇΓöÇ Clear all voxels ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Render ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  return (
    <div
      style={{
        position: 'absolute',
    ...
    >
      {/* ΓöÇΓöÇ 3D Viewport ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <div
        ref={canvasRef}
      ...
      {/* ΓöÇΓöÇ Sidebar Panel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <div
        style={{
          width: 300,
      ...
      >
        {/* Header */}
        <div>
          <div style={{ fontSize: 20, fontWeight: 900, color: '#4488ff', letterSpacing: 3, marginBottom: 4 }}>SHIP FORGE</div>
          <div style={{ fontSize: 10, color: 'rgba(160,200,255,0.4)', letterSpacing: 2 }}>DESIGN YOUR HERO SHIP</div>
        </div>
        ...
        {/* ΓöÇΓöÇ PREFAB MODE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {mode === 'prefab' && (
          <>
            <div style={{ fontSize: 8, color: 'rgba(160,200,255,0.25)', lineHeight: 1.6 }}>
              Click part ΓåÆ click grid to place ┬╖ Click placed part to select
              <br />
              ...
              <div key={cat.key}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {SHIP_PARTS.filter((p) => p.category === cat.key).map((part) => (
                    ...
                    >
                      <div
                        style={{ width: 8, height: 8, borderRadius: 2, background: part.color, display: 'inline-block', marginRight: 4 }}
                      />
                      {part.name}
            ...
            {/* Gizmo mode selector */}
            <div>
              <div style={{ fontSize: 9, color: 'rgba(160,200,255,0.4)', fontWeight: 700, letterSpacing: 1.5, marginBottom: 4 }}>
                GIZMO MODE
              </div>
              <div style={{ display: 'flex', gap: 4 }}>
                {(['translate', 'rotate', 'scale'] as const).map((m) => (
                  <Btn
                    key={m}
                    label={m === 'translate' ? 'G Move' : m === 'rotate' ? 'R Rotate' : 'S Scale'}
                    active={gizmoMode === m}
              ...
              <div style={{ padding: '8px 10px', background: 'rgba(68,136,255,0.08)', border: '1px solid #4488ff44', borderRadius: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  <Btn
                    ...
                    style={{ flex: 1, height: 28, minWidth: 0 }}
                  />
                  <Btn label="DESELECT" onClick={() => setSelectedPlacedId(null)} style={{ flex: 1, height: 28, minWidth: 0 }} />
                </div>
              </div>
            )}
        ...
        {/* ΓöÇΓöÇ VOXEL MODE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {mode === 'voxel' && (
          <>
            <div style={{ fontSize: 8, color: 'rgba(160,200,255,0.25)', lineHeight: 1.6 }}>
              Left-click: place/edit ┬╖ Right-drag: orbit ┬╖ Scroll: zoom
              <br />
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {(['place', 'erase', 'paint'] as Tool[]).map((t) => (
                  <button
                    ...
                    style={{
                      ...toolBtn,
                      background: tool === t ? '#4488ff' : 'transparent',
            ...
            >
              {mirror ? 'Γçö Mirror ON' : 'Γçö Mirror OFF'}
            </button>
            <div style={{ padding: '8px 10px', background: 'rgba(2,4,12,0.6)', borderRadius: 6, border: '1px solid #1a3050' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10 }}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {message && (
            <div style={{ fontSize: 11, color: message.includes('fail') ? '#ff4444' : '#44dd88', textAlign: 'center' }}>{message}</div>
          )}
          <button
            onClick={handleSave}
            disabled={saving}
...
// ΓöÇΓöÇ Styles ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
