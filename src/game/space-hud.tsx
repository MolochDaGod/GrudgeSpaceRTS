// space-hud.tsx ΓÇö Main SpaceHUD component
...
export function SpaceHUD({ renderer, onQuit, onToggleStarMap, onDeployGround }: SpaceHUDProps) {
  ...
  // Primary selected ship ΓÇö use getShipDef so Hero ships resolve correctly
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', fontFamily: "'Segoe UI', monospace", color: '#cde' }}>
      {/* ΓöÇΓöÇ Game Over Overlay with Victory/Defeat badge art ΓöÇΓöÇΓöÇΓöÇ */}
      {state.gameOver && (
        <div
          style={{
            position: 'absolute',
      ...
      {/* ΓöÇΓöÇ Domination Warning ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Game Alerts with cyber avatar popups ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Info Bot (first 30 seconds of game) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Top Resource Bar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <div
        style={{
          position: 'absolute',
          top: 0,
      ...
      >
        ...
        />
        {/* Resources with income rates ΓÇö using ResBox from ui-lib */}
        {(() => {
          let incC = 0,
            incE = 0,
            incM = 0;
      ...
      {/* ΓöÇΓöÇ Production Sidebar (left) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {!state.gameOver && <ProductionSidebar state={state} renderer={renderer} />}

      {/* ΓöÇΓöÇ Void Power Castbar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Captain's Journal (L key) ΓÇö new sci-fi panel ΓöÇΓöÇ */}
      {logOpen && <JournalPanel state={state} onClose={() => setLogOpen(false)} />}

      {/* ΓöÇΓöÇ Commander Character Panel (C key) ΓöÇΓöÇ */}
      {charPanelOpen && (
      ...
      {/* ΓöÇΓöÇ Unified Skill Tree (K key) ΓöÇΓöÇ */}
      {skillTreeOpen && (
      ...
      {/* ΓöÇΓöÇ Floating panels ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {techOpen && (
        <TechTreePanel
          state={state}
          myPlanets={myPlanets}
      ...
      {/* ΓöÇΓöÇ Planet Card (planet click / F1) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {planetCardOpen &&
      ...
      {/* ΓöÇΓöÇ Planet Surface View (BUILD from planet card) ΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ ENTER SHIP popup (flagship selected) ΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Flagship Interior Overlay ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {interiorOpen &&
        (() => {
      ...
      {/* ΓöÇΓöÇ Floating Damage Numbers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {renderer &&
      ...
      {/* ΓöÇΓöÇ Capture Progress Labels on Planets ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {renderer &&
        state.planets
      ...
      {/* ΓöÇΓöÇ Bottom Panel ΓÇö SC2-style: [minimap] [portrait+stats] [command grid] ΓöÇΓöÇ */}
      <div
        style={{
          position: 'absolute',
          bottom: 0,
      ...
      >
        {/* Single background layer ΓÇö no stacking */}
        <img
        ...
        {/* ΓöÇΓöÇ Region 1: Minimap ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div
          style={{
            width: 200,
            flexShrink: 0,
        ...
        {/* ΓöÇΓöÇ Region 2: Portrait + Unit Stats ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div
          style={{
        ...
        >
          {/* Portrait (left half) ΓÇö uses space-shooter-gui avatar frame */}
          <div
            style={{
              width: 160,
              flexShrink: 0,
          ...
          >
            {primary && def ? (
              <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                ...
                />
                {/* Ship preview image ΓÇö fills the portrait square area */}
                <div
                  style={{
                    position: 'absolute',
                    left: 4,
                ...
                >
                  ...
                  <div style={{ fontSize: 8, color: 'rgba(160,200,255,0.5)', display: 'flex', gap: 4, alignItems: 'center' }}>
                    ...
                    {primary.rank > 0 && <span style={{ color: '#ffcc00' }}>{'Γÿà'.repeat(primary.rank)}</span>}
                  </div>
                </div>
              </div>
            ) : (
        ...
        {/* ΓöÇΓöÇ Region 3: Sci-Fi Command Card + Fleet Bar ΓöÇΓöÇ */}
        <div
          style={{
            flex: 1,
            minWidth: 200,
        ...
        >
          {/* Fleet Bar ΓÇö commander + 12 ships */}
          <FleetBar state={state} />

          {/* Command Card ΓÇö sci-fi bar with 4 commands + 4 skills + stop */}
          {primary ? (
            <SciFiCommandCard
              ship={primary}
              onCommand={(cmd) => {
        ...
        {/* ΓöÇΓöÇ Region 4: Menu Button Bar (right strip) ΓöÇΓöÇ */}
        <div style={{ flexShrink: 0, zIndex: 1, display: 'flex', alignItems: 'flex-start', paddingTop: 4 }}>
          <MenuButtonBar
            onAction={(id) => {
              ...
              else if (id === 'mail') console.log('[menu] AI Chat ΓÇö coming soon');
              else if (id === 'databook') console.log('[menu] Codex ΓÇö coming soon');
            }}
          />
        </div>
      </div>
